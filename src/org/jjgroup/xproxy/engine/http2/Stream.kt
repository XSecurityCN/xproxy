...
class Stream(val connection: H2Connection, val streamID: Int, val req: Request, fromClient: Boolean) {
    ...
    var state = CLEAN
    var headers: String = ""
    // 用 StringBuilder 累积多帧 body,避免 String += 的 O(n²) 拷贝。
    ...
    fun processFrame(frameBytes: ByteArray) {
        ...
        if (state == CLEAN) {
            when (frame) {
                ...
                is DataFrame -> throw Exception("Expected headers, got data")
            }
        }
...
//            if (HttpRequestEngine.shouldUngzip(headers)) {
//                body = HttpRequestEngine.ungzip(body.toByteArray(Charsets.ISO_8859_1))
//            }
