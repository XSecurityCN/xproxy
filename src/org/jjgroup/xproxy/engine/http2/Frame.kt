...
open class Frame(val type: Byte, val flags: Byte, val streamID: Int, val payload: ByteArray) {
    ...
    fun asBytes(): ByteArray {
        ...
        val streamIdBytes = HTTP2Utils.intToFourBytes(streamID)
        // 预分配 9+payload.size 字节并填入,避免 5 段 ByteArray 用 + 拼接产生多次中间数组分配。
        val result = ByteArray(9 + payload.size)
        System.arraycopy(length, 0, result, 0, 3)
        result[3] = type
        result[4] = flags
        System.arraycopy(streamIdBytes, 0, result, 5, 4)
...
class HTTP2Utils {
    companion object {
        ...
        fun intToThreeBytes(num: Int): ByteArray {
            val s2 = num shr 8
            val s3 = num shr 16
            return byteArrayOf(s3.toByte(), s2.toByte(), num.toByte())
        }
        ...
        fun intToFourBytes(num: Int): ByteArray {
            ...
            val s3 = num shr 16
            val s4 = num shr 24
            return byteArrayOf(s4.toByte(), s3.toByte(), s2.toByte(), num.toByte())
        }
    }
}
