package org.jjgroup.xproxy.engine.http2
...
class H2Connection(val target: URL, val seedQueue: Queue<Request>, private val requestQueue: LinkedBlockingQueue<Request>, var requestsPerConnection: Int, val engine: HTTP2RequestEngine) {
    ...
    companion object {
        ...
        // 每个 H2Connection(重)连接原本都新建 SSLContext+init;TrustingTrustManager 无状态,进程级共享一次复用。
        private val SHARED_SSL_CONTEXT: SSLContext by lazy {
        ...
        fun buildReqFromRaw(rawRequest: String, transform: Boolean = true): LinkedList<Pair<String, String>> {
            return buildReq(HTTP2Request(rawRequest), transform)
        }
        ...
        fun buildReq(parsedRequest: HTTP2Request, transform: Boolean): LinkedList<Pair<String, String>> {
            ...
            if (!pseudoHeaders.containsKey(":scheme")) {
                result.add(Pair(":scheme", "https"))
            }
            if (!pseudoHeaders.containsKey(":method")) {
                result.add(Pair(":method", parsedRequest.method))
            }
            if (!pseudoHeaders.containsKey(":path")) {
                result.add(Pair(":path", parsedRequest.path))
            }
            if (!pseudoHeaders.containsKey(":authority")) {
                result.add(Pair(":authority", host))
    ...
    var done = false
    // 被 readForever / writeForever 线程并发读写,改用 ConcurrentHashMap 避免普通 HashMap 在 resize 时的死循环/丢数据。
    val streams: ConcurrentHashMap<Int, Stream> = ConcurrentHashMap()
    ...
    private fun connect() {
        ...
        if (UpstreamProxySettings.isEnabled() && upstream == null) {
            throw IllegalStateException("Upstream proxy is enabled but host/port is invalid")
        }

        when {
            upstream != null && upstream.protocol == UpstreamProxyProtocol.SOCKS5 -> {
                val proxySocket = Socket(Proxy(Proxy.Type.SOCKS, InetSocketAddress(upstream.host, upstream.port)))
                proxySocket.connect(InetSocketAddress(target.host, port), 10000)
                if (target.protocol == "https") {
                    val sslsf = createSSLSocketFactory()
                    socket = sslsf.createSocket(proxySocket, target.host, port, true) as SSLSocket
            ...
            upstream != null -> {
                ...
                establishConnectTunnel(tunnelSocket, target.host, port, upstream.proxyAuthorizationHeaderValue())
                if (target.protocol == "https") {
                    val sslsf = createSSLSocketFactory()
                    socket = sslsf.createSocket(tunnelSocket, target.host, port, true) as SSLSocket
                    val sslp = (socket as SSLSocket).sslParameters
                    val clientAPs = arrayOf("h2")
                    sslp.applicationProtocols = clientAPs
                    (socket as SSLSocket).sslParameters = sslp
    ...
    private fun readForever() {
        try {
            ...
            while (checkState() != CLOSED) {
                ...
                if (streams.isEmpty() && state != CONNECTING) {
                    Thread.sleep(100)
                    continue
                }
    ...
    fun hasInflightRequests() = streams.isNotEmpty() && (streams.size > 1 || !streams.containsKey(0))
...
//    }
    ...
    private fun createSSLSocketFactory(): SSLSocketFactory {
        return SHARED_SSL_CONTEXT.socketFactory
    }
