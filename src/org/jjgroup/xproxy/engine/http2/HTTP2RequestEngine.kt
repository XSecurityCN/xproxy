...
open class HTTP2RequestEngine(
    url: String,
    val threads: Int,
    maxQueueSize: Int,
    var requestsPerConnection: Int,
...
//                }
                ...
                if (con.state == H2Connection.CLOSED) {
                    if (con.hasInflightRequests() || con.seedQueue.size > 0 || attackState.get() < 3) {
                        ...
                        connections.incrementAndGet()
                        connectionPool[i] = H2Connection(target, seedQueue, requestQueue, requestsPerConnection, this)
                    }
                }
            }
            Thread.sleep(10)
    ...
    fun prepareHeaders(rawRequest: String): List<Pair<String, String>> {
        return H2Connection.buildReqFromRaw(rawRequest)
    }
